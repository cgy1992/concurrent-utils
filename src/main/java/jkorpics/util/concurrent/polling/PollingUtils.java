...
public class PollingUtils {
    ...
    /**
     * Poll condition at regular intervals until the condition is true or until a max time limit is reached.
     * This method can help in unit testing rather than just sleeping, which can cause random test failures.
     ...
     */
    public static <T> boolean waitForPredicate(final Predicate<T> condition,
                                               T object,
                                               long intervalMs,
                                               long maxTimeMs) {
    ...
    /**
     * Much like waitForPredicate except the condition function does not require an argument when called.
     * @param condition - the condition function to call at regular intervals
     * @param intervalMs - the interval in ms between each condition check.
     * @param maxTimeMs - max amount of time in ms to wait for the condition\
     * @return - True if the condition is met within the given timeframe, False otherwise.
     */
    public static <T> boolean waitForSupplier(final Supplier<Boolean> condition,
                                              long intervalMs,
                                              long maxTimeMs) {
        long totalWaitTime = 0L;
